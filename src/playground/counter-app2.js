class Counter extends React.Component {
  ...
  componentDidMount() {
    try {
      ...
      const counter = parseInt(strCount);
      if(!isNaN(counter)){ // If there was already a counter in local storage, then use it to set the state.
  ...
  componentDidUpdate(prevState){
    if(prevState.counter !== this.state.counter){
      const curCount = this.state.counter; // Get current options.
      localStorage.setItem("counter", curCount)
    }
  }
